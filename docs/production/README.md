...
| 3 | Restart/reboot survival and manual resource baseline | **OPS-5 is complete:** a controlled reboot proved automatic recovery and an existing authenticated BFF filesystem session survived. The verified manual baseline is run after controlled restart, monthly and on symptoms; see [operations.md §8](operations.md#8-restart-reboot-and-resource-monitoring). |
...
OPS-5 provides only the verified **manual** resource baseline in
[operations.md §8](operations.md#8-restart-reboot-and-resource-monitoring).
