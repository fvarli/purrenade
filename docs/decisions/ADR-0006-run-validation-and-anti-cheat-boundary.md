...
- The game domain is **pure, deterministic and portable** —
  [ADR-0002](ADR-0002-nuxt-phaser-frontend-architecture.md) — which keeps several
...
[`../product/achievements-and-unlocks.md`](../product/achievements-and-unlocks.md) §1.5. They
...
[`../product/achievements-and-unlocks.md`](../product/achievements-and-unlocks.md) §1.5A.
...
property it would add. The backend's `docs/security/threat-model.md` §4 answers *forged
...
`app/composables/useRunSurface.ts` is still browser-generated via `Math.random()`; its own
