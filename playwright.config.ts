...
export default defineConfig({
  ...
  projects: [
    ...
    },
    {
      ...
      name: 'chrome',
      testIgnore: [/auth\.setup\.ts/, /run-surface\.spec\.ts/, /run-lifecycle\.spec\.ts/, /run-obstacles\.spec\.ts/, /run-scoring\.spec\.ts/, /run-tutorial\.spec\.ts/, /run-submission\.spec\.ts/, /run-touch\.spec\.ts/, /ssr-auth-session\.spec\.ts/],
      use: {
    ...
    },
    {
      ...
      name: 'chrome-auth',
      testMatch: /(run-(surface|lifecycle|obstacles|scoring|tutorial|submission)|ssr-auth-session)\.spec\.ts/,
      dependencies: ['setup'],
      use: {
        channel: 'chrome',
